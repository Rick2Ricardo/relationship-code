...
class FrameWithTwoEncoders(object):
    ...
    #     # 4) 使用 SpectralClustering 进行聚类
    #     clustering = SpectralClustering(
    #         n_clusters=num_clusters, 
    #         affinity='nearest_neighbors',  # 选择适合的 affinity
    #         random_state=0
    #     ).fit(features)

    #     labels = clustering.labels_  # 每个样本的簇标签
    ...
    #         center_index = cluster_indices[np.argmin(np.linalg.norm(cluster_features - cluster_features.mean(axis=0), axis=1))]

    #         # 添加选定的样本
    #         mem_set.append(sample_set[center_index])
    #         current_feat.append(features[center_index])
